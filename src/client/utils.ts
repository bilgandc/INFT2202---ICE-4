...
export function validateInput(fieldId: string): boolean {
    ...
    const errorElement = document.getElementById(`${fieldId}-Error`)as HTMLInputElement| null;
...
export function getFormStorage<T>(key: string): T | null {
    try {
        ...
        if (key.startsWith("contact_")) {
            ...
            return contact as unknown as T; //cast contact to generic T
        }
...
export function removeFormStorage(key: string): void {
    try {
